...
public class eeuiRongmModule extends WXModule {
    ...
    public void login(String object, final JSCallback callback) {
        ...
        //
        final UserInfo user = new UserInfo(json.getString("userid"), eeuiJson.getString(json, "username", json.getString("userid")), Uri.parse(eeuiJson.getString(json, "userimg")));
        FakeServer.getToken(user, new HttpUtil.OnResponse() {
            @Override
            public void onResponse(int code, String body) {
                if (code != 200) {
                    JSONObject json = eeuiJson.parseObject(body);
                    data.put("status", "error");
                    data.put("errorCode", eeuiJson.getInt(json, "code", -801));
                    data.put("errorMsg", eeuiJson.getString(json, "errorMessage", body));
                    invoke(callback, data);
                    return;
                }
                //
                final String token = body;
                eeui_rongim.connect(body, new RongIMClient.ConnectCallback() {
                    @Override
                    ...
                    public void onSuccess(String userId) {
                        eeui_rongim.setCurrentUser(user);
                        //
    ...
    public void joinChatRoom(String roomId, int defMessageCount, final JSCallback callback) {
        ...
        data.put("errorMsg", "");
        //
        eeui_rongim.joinChatRoom(roomId, defMessageCount, new RongIMClient.OperationCallback(){
            @Override
    ...
    public void quitChatRoom(final JSCallback callback) {
        ...
        data.put("errorMsg", "");
        //
        eeui_rongim.quitChatRoom(new RongIMClient.OperationCallback() {
            @Override
            public void onSuccess() {
                data.put("status", "success");
                invoke(callback, data);
        ...
        if (mEventHandler == null) {
            mEventHandler = new Handler(new Handler.Callback() {
                ...
                public boolean handleMessage(Message msg) {
                    switch (msg.what) {
                        case eeui_rongim.MESSAGE_ARRIVED: {
                        ...
                        case eeui_rongim.MESSAGE_SEND_ERROR: {
                            if (msg.obj instanceof TextMessage) {
                                TextMessage message = (TextMessage) msg.obj;
                                MessageContent content = (MessageContent) msg.obj;
                                UserInfo mUserInfo = content.getUserInfo();
            ...
            eeui_rongim.addEventHandler(mEventHandler);
        }
    }
    ...
    public void removeEventHandler() {
        ...
        eeui_rongim.removeEventHandler(mEventHandler);
        mEventHandler = null;
    }
    ...
    public void sendTextMessage(String text, final JSCallback callback) {
        ...
        eeui_rongim.sendMessage(TextMessage.obtain(text), new IRongCallback.ISendMessageCallback() {
            @Override
