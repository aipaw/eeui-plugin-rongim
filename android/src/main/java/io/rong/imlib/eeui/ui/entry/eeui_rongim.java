package io.rong.imlib.eeui.ui.entry;
...
import io.rong.imlib.eeui.ui.module.eeuiRongmModule;
...
public class eeui_rongim {
    ...
    public void init(Context content) {
        JSONObject rongim = eeuiJson.parseObject(eeuiBase.config.getObject("rongim").get("android"));
        if (eeuiJson.getBoolean(rongim, "enabled")) {
            eeui_rongim.init(eeuiJson.getString(rongim, "appKey"), eeuiJson.getString(rongim, "appSecret"));
        }
        //注册模块
        if (!isRegister) {
            isRegister = true;
    ...
    public static void init(String appKey, String appSecret) {
        if (appKey != null && !"".equals(appKey)) {
            eeui_rongim.appKey = appKey;
        }
        if (appSecret != null && !"".equals(appSecret)) {
            eeui_rongim.appSecret = appSecret;
        }
        RongIMClient.init(eeui.getApplication(), appKey);
